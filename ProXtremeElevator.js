...
class ElevatorSystem {
    constructor(floorArray, initialFloor) {
        this.elevator1 = new Elevator(floorArray, initialFloor);
        this.elevator2 = new Elevator(floorArray, initialFloor);
    }

    requestElevator(floor) {
        const distance1 = Math.abs(this.elevator1.currentFloor - floor);
        const distance2 = Math.abs(this.elevator2.currentFloor - floor);

        if (distance1 <= distance2) {
            this.elevator1.requestElevator(floor);
        } else {
            this.elevator2.requestElevator(floor);
        }
    }

    runElevators() {
        while (true) {
            this.elevator1.runElevator();
            this.elevator2.runElevator();

            if (
                this.elevator1.floorArray.length === 0 &&
                Object.keys(this.elevator1.enteredFloors).length === 0 &&
                this.elevator2.floorArray.length === 0 &&
                Object.keys(this.elevator2.enteredFloors).length === 0
            ) {
                break;
            }
        }
...
let elevatorSystem = new ElevatorSystem(floorArray, initialFloor);
...
elevatorSystem.runElevators();

console.log(elevatorPanel);
